...
class Karisik extends StatefulWidget {
  @override
  _KarisikState createState() => _KarisikState();
}

class _KarisikState extends State<Karisik> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<List<Kelime>> veri){
        ...
        kelimeler =veri.data;
        if(ingilizce == null){
          int sayac =randomUret(veri.data);
        ...
        return Form(
          ...
          child: Padding(
              ...
              child: Column(
                  ...
                  children: <Widget>[
                    isIng == true ? Text('$ingilizce',style: TextStyle(fontSize: 22.0),) : Text('$turkce',style: TextStyle(fontSize: 22.0),),
                    SizedBox(height: 25.0,),
                    TextFormField(
                      controller: _clearControl,
                      decoration: InputDecoration(
                         hintText: isIng == true ? 'Türkçesini Yazınız' : 'İngilizcesini Yazınız',
          ...
          child: RaisedButton(
            onPressed: () async{
              if(_kontrol.currentState.validate()){
                ...
                if(isIng)
                  await cevapKontrolTur(context);
                else
                  await cevapKontrolIng(context);
  ...
  Future<void> cevapKontrolTur(BuildContext context) async{

    print('türkçe tahmin kontrol ediliyor turkce :$turkce tahminT:$tahminTurk ingilizce:$ingilizce tahmining: $tahminIng');
    if(turkce ==tahminTurk){
      //List<Kelime> kelimeler = await _dbHelper.getKelimeler(false);
      int index = randomUret(kelimeler);
      Scaffold.of(context).showSnackBar(SnackBar(
        content: Text('$turkce kelimesini Doğru bildiniz',style: TextStyle(color: Colors.white,fontSize: 17.0),textDirection: TextDirection.rtl,),
      ...
      int index = randomUret(kelimeler);
      Scaffold.of(context).showSnackBar(SnackBar(
        content: Text('Cevap $ingilizce',style: TextStyle(color: Colors.white,fontSize: 17.0),textDirection: TextDirection.rtl,),
  ...
  Future<void> cevapKontrolIng(BuildContext context) async{

    print('ting: $tahminTurk');
    if(ingilizce ==tahminIng){
      //List<Kelime> kelimeler = await _dbHelper.getKelimeler(false);
      int index = randomUret(kelimeler);
      Scaffold.of(context).showSnackBar(SnackBar(
        content: Text('$ingilizce kelimesini Doğru bildiniz',style: TextStyle(color: Colors.white,fontSize: 17.0),textDirection: TextDirection.rtl,),
        backgroundColor: Colors.blue,
        duration: Duration(milliseconds: 1200)
      ));
      setState(() {
